...
export default class LessEqual extends ComparisonExpression {
  ...
  get type() {
    return ExpressionTypes.LessEqual;
  }
  ...
  static evaluate(expression: Expression): Tensor {
    let node = expression as LessEqual;
    let left = node.left.value;
    let right = node.right.value;
    return TensorMath.lessEqual(left, right);
  }
