...
export default class GreaterEqual extends ComparisonExpression {
  ...
  get type() {
    return ExpressionTypes.GreaterEqual;
  }
  ...
  static evaluate(expression: Expression): Tensor {
    let node = expression as GreaterEqual;
    let left = node.left.value;
    let right = node.right.value;
    return TensorMath.greaterEqual(left, right);
  }
